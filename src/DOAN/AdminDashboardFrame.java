...
class AdminDashboardFrame extends JFrame {
    ...
    public AdminDashboardFrame() {
        ...
        // 1. Menu "Trang Chủ" vẫn giữ nguyên
        ...
        JMenu menuQLPhim = createMenu("Quản Lý Phim", () -> {
            dispose();
            new AdminFilmsFrame();
        });

        JMenu menuQLVe = createMenu("Quản Lý Vé", () -> {
            dispose();
            new AdminViewBookingFrame();
        });

        JMenu menuQLKhachHang = createMenu("Quản Lý Khách Hàng", () -> {
            dispose();
            new AdminCustomerFrame();
        });

        JMenu menuQLKhuyenMai = createMenu("Quản Lý Khuyến Mãi", () -> {
            dispose();
            new AdminPromotionsFrame();
        });
        
        JMenu menuThongKe = createMenu("📊 Quản Lý Thống Kê", () -> {
            dispose();
            new AdminStatisticsFrame();
        });

        // Thêm các menu mới vào thanh menu
        menuBar.add(menuQLPhim);
        menuBar.add(menuQLVe);
        menuBar.add(menuQLKhachHang);
        menuBar.add(menuQLKhuyenMai);
        // Gắn menuBar vào frame
        setJMenuBar(menuBar);
